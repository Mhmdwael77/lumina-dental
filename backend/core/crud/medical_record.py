"""
Database query helpers for standalone medical records and their images.
"""

from sqlalchemy import or_
from sqlalchemy.orm import Session

from core.database import MedicalRecord, MedicalImage
    ...
    if search:
        ...
        q = q.filter(
            or_(
                ...
                MedicalRecord.diagnosis.ilike(like),
            )
        )
...
def delete_record(db: Session, record_id: int) -> bool:
    ...
    if record is None:
        return False
    db.delete(record)  # cascade removes image rows
    db.commit()
    return True


def add_image(db: Session, record_id: int, *, filename: str, original_name: str | None, content_type: str | None) -> MedicalImage:
    image = MedicalImage(
        record_id=record_id,
        filename=filename,
